...
struct Creature
{
    ...
    private:
        ...
        struct special_abilities
        {
            ...
            static constexpr uint8_t bits = CHAR_BIT * sizeof(m_abilities[0]); // size of each int in the m_abilities array
            ...
            inline bool get_ability_bit(Abilities bit_pos) const { return m_abilities[bit_pos / bits] &   (1 << (bit_pos % bits)); }
            inline void set_ability_bit(Abilities bit_pos)       {        m_abilities[bit_pos / bits] |=  (1 << (bit_pos % bits)); }
            inline void turn_off_ability_bit(Abilities bit_pos)  {        m_abilities[bit_pos / bits] &= ~(1 << bit_pos);          }

            // Create a map of all special abilities, refering to the fields in the struct accordingly.
            const std::map< std::string, std::vector<Abilities> >& create_map_of_all_abilities();
