...
{
    ASSERT( get_attacker()->get_team() != get_defender()->get_team(), "Two heroes of the same team cannot go to war with each other!" );
...
{
      ASSERT( Hero_slots::ARMY == 7, "Positioning is made for 7 slots in a hero's army! If you'd like to use different number of slots you need to change the positioning and the battle map width and length." );
...
void Battle::inflict_damage(Stack* const attacking_stack, Stack* const defending_stack, bool const defender_is_targeted, bool const attack_is_retaliation, bool const attack_is_second_attack)
{
    ...
    else if( attacking_creature->get_hates_genies()        && ( defending_creature == Creature::get(Genie)  || defending_creature == Creature::get(Master_Genie) )  ) I5 = PERCENT_50;
    else if( attacking_creature->get_hates_devils()        && ( defending_creature == Creature::get(Devil)  || defending_creature == Creature::get(Arch_Devil) )    ) I5 = PERCENT_50;
    else if( attacking_creature->get_hates_angels()        && ( defending_creature == Creature::get(Angel)  || defending_creature == Creature::get(Archangel) )     ) I5 = PERCENT_50;
    else if( attacking_creature->get_hates_black_dragons() &&   defending_creature == Creature::get(Black_Dragon) ) I5 = PERCENT_50;
    else if( attacking_creature->get_hates_titans()        &&   defending_creature == Creature::get(Titan)        ) I5 = PERCENT_50;

    // elementals bonus
    if     ( ( attacking_creature == Creature::get(Air_Elemental)   || attacking_creature == Creature::get(Storm_Elemental)  ) && ( defending_creature == Creature::get(Earth_Elemental) || defending_creature == Creature::get(Magma_Elemental)  ) ) I5 = PERCENT_100;
    else if( ( attacking_creature == Creature::get(Water_Elemental) || attacking_creature == Creature::get(Ice_Elemental)    ) && ( defending_creature == Creature::get(Fire_Elemental)  || defending_creature == Creature::get(Energy_Elemental) ) ) I5 = PERCENT_100;
    else if( ( attacking_creature == Creature::get(Fire_Elemental)  || attacking_creature == Creature::get(Energy_Elemental) ) && ( defending_creature == Creature::get(Water_Elemental) || defending_creature == Creature::get(Ice_Elemental)    ) ) I5 = PERCENT_100;
    else if( ( attacking_creature == Creature::get(Earth_Elemental) || attacking_creature == Creature::get(Magma_Elemental)  ) && ( defending_creature == Creature::get(Air_Elemental)   || defending_creature == Creature::get(Storm_Elemental)  ) ) I5 = PERCENT_100;
    ...
    if     ( attacking_creature == Creature::get(Psychic_Elemental) && defending_creature->get_is_immune_to_mind_spells() ) R8 = PERCENT_50;
    else if( attacking_creature == Creature::get(Magic_Elemental)   && defending_creature->get_is_immune_to_all_spells()  ) R8 = PERCENT_50;
...
{
    if( !defending_stack->get_creature()->get_has_no_enemy_retaliation() )
